		...
		*/
		...
		public:
			...
			*/
			...
			inline static Message<ValueType> *Create(IAllocator *const messageAllocator, const ValueType &value, const Address &from);

			/**
			Destructs and frees a message of unknown type referenced by an interface pointer.
			*/
			inline static void Destroy(IAllocator *const messageAllocator, IMessage *const message);
		};


		template <class ValueType>
		THERON_FORCEINLINE Message<ValueType> *MessageCreator::Create(IAllocator *const messageAllocator, const ValueType &value, const Address &from)
		{
		...
		THERON_FORCEINLINE void MessageCreator::Destroy(IAllocator *const messageAllocator, IMessage *const message)
		{
